...
<script>
  ...
  export default {
    ...
    title() {
      return {
        title: (this.articleListData.entity.info.name ? this.articleListData.entity.info.name : '博文') + ' - Flost 的小窝'
      }
    },
    computed: {
      ...mapGetters({
        articleListData: 'GetArticleListData'
      })
    },
    asyncData({ app, route, store }) {
      ...
      return store.dispatch('GetArticleListData', data)
    },
    methods: {
      formatDate(val) {
      ...
      getArticleList() {
        ...
        that.$API.getArticleList({
          ...
          success(res) {
            if (res.code === 10000) {
              that.$data.page = res.entity.page
              if (that.$data.loadMore) {
                that.$data.articles = [...that.$data.articles, ...res.entity.data]
              } else {
                that.$data.articles = res.entity.data
              }
              that.$store.dispatch('setSiteTitle', (res.entity.info.name ? res.entity.info.name : '博文') + ' - Flost 的小窝')
              if (that.$data.currentPage >= that.$data.page.pageCount) {
    ...
    created () {
      ...
      if (that.articleListData.code === 10000) {
        that.$data.articles = that.articleListData.entity.data
        that.$data.page = that.articleListData.entity.page
      }
    }
  }
</script>

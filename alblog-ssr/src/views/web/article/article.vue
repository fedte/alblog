  ...
  <section>
          ...
          <div class="digg flex-center mt10">
            ...
            <template v-if="isClient">
              <el-popover
                ...
                <span class="c-666 hover" slot="reference" v-popover:rewards title="打赏作者">赏</span>
              </el-popover>
            </template>
          </div>
        </section>
    ...
    <section class="content-comment mt10 bgc-fff" ref="comment">
      <div class="flex header bgc-fff" v-if="commentsList !== null">
        ...
        <div class="editbox ml10" :class="!showPlaceholder ? 'open' : ''" v-if="isClient">
          <el-input
            ref="commentsBox"
            type="textarea"
            :rows="3"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        articleInfoData: 'GetArticleInfoData',
        articleCommentList: 'GetArticleCommentData'
      })
    },
    title() {
      return {
        title: this.articleInfoData.entity.article.title,
        keywords: this.articleInfoData.entity.article.tag.join(','),
        description: this.$utils.parseDesc(this.articleInfoData.entity.article.content)
      }
    },
    watch: {
      'reply' (val) {
    ...
    asyncData({ app, store, route }) {
      return [
        store.dispatch('GetArticleInfoData', {
          id: route.params.id
        }),
        store.dispatch('GetArticleCommentData', {
          id: route.params.id,
          limit: 10,
    ...
    methods: {
      ...
      getArticleInfo() {
        ...
        that.$API.getArticleInfo({
          ...
          success(res) {
            ...
            console.log('------ End------')
            if (res.code === 10000) {
              that.$data.article = res.entity.article
          ...
          success(res) {
            if (res.code === 10000) {
              if (that.$data.loadMore) {
                ...
                that.$data.commentsList = [...that.$data.commentsList, ...res.entity.list]
              } else {
                that.$data.commentsList = res.entity.list
              }
              that.$data.page = res.entity.pages
              if (that.$data.currentPage >= that.$data.page.pageCount) {
          ...
          success(res) {
            if (res.code === 10000) {
              ...
              setTimeout(() => {
                ...
                that.$data.commentsList.unshift(res.entity.comment)
              }, 1000)
      ...
      goComment() {
        window.scrollTo(0, this.$refs.comment.offsetTop - 100)
      },
      articleDigg() {
        let that = this
        console.log('--------点赞---------')
    ...
    async created() {
      ...
      if (that.articleInfoData.code === 10000) {
        that.$data.article = that.articleInfoData.entity.article
        that.$data.commentsList = that.articleCommentList.entity.list
        that.$data.page = that.articleCommentList.entity.pages
        if (that.$data.currentPage >= that.$data.page.pageCount) {
          that.$data.loadAll = true
        } else {
          that.$data.loadAll = false
    ...
    async mounted () {
      let that = this
      that.$data.copyrightUrl = window.location.href
      that.$data.isClient = true
    }
  }
</script>

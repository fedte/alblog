...
CKEDITOR.dialog.add("scaytcheck", function (j) {
  function w() {
    ...
    if (a) {
      var e = a.length;if (void 0 == e) a.checked = a.value == b.toString();else for (var d = 0; d < e; d++) a[d].checked = !1, a[d].value == b.toString() && (a[d].checked = !0);
    }
  }function n(a) {
    ...
    label: g.aboutTab, elements: [{ type: "html", id: "about", style: "margin: 5px 5px;", html: '<div><div id="scayt_about_' + b + '"></div></div>' }] }],
      ...
      var a = this;a.data = j.fire("scaytDialog", {});a.options = a.data.scayt_control.option();a.chosed_lang = a.sLang = a.data.scayt_control.sLang;if (!a.data || !a.data.scayt || !a.data.scayt_control) alert("Error loading application service"), a.hide();else {
        var b = 0;s ? a.data.scayt.getCaption(j.langCode || "en", function (e) {
          0 < b++ || (h = e, A.apply(a), y.apply(a), s = !1);
        }) : y.apply(a);a.selectPage(a.data.tab);
      }
    }, onOk: function () {
      var a = this.data.scayt_control;a.option(this.options);a.setLang(this.chosed_lang);a.refresh();
    }, onCancel: function () {
      var a = w(),
          f;for (f in a) a[f].checked = !1;a = "undefined" != typeof document.forms["languagesbar_" + b] ? document.forms["languagesbar_" + b].scayt_lang : [];x(a, "");
    }, contents: t };CKEDITOR.plugins.scayt.getScayt(j);for (g = 0; g < l.length; g++) 1 == l[g] && (t[t.length] = z[g]);1 == l[2] && (u = 1);var A = function () {
    function a(a) {
      var c = f.getById("dic_name_" + b).getValue();if (!c) return n(" Dictionary name should not be empty. "), !1;try {
        var d = a.data.getTarget().getParent(),
            e = /(dic_\w+)_[\w\d]+/.exec(d.getId())[1];j[e].apply(null, [d, c, m]);
      } catch (C) {
        n(" Dictionary error. ");
      }return !0;
    }var k = this,
        ...
        i = [],
        c;if (u) {
      for (c = 0; c < d.length; c++) g[c] = d[c] + "_" + b, f.getById(g[c]).setHtml('<span class="cke_dialog_ui_button">' + h["button_" + d[c]] + "</span>");f.getById("dic_info_" + b).setHtml(h.dic_info);
      ...
      var c = f.createElement("label");c.setAttribute("for", "cke_option" + a);c.setStyle("display", "inline");c.setHtml(b[a]);k.sLang == a && (k.chosed_lang = a);var d = f.createElement("div"),
          e = CKEDITOR.dom.element.createFromHtml('<input class = "cke_dialog_ui_radio_input" id="cke_option' + a + '" type="radio" ' + (k.sLang == a ? 'checked="checked"' : "") + ' value="' + a + '" name="scayt_lang" />');e.on("click", function () {
        this.$.checked = true;k.chosed_lang = a;
      });d.append(e);d.append(c);return { lang: b[a], code: a, radio: d };
    };if (1 == l[1]) {
      for (c in e.rtl) i[i.length] = d(c, e.ltr);for (c in e.ltr) i[i.length] = d(c, e.ltr);i.sort(function (a, b) {
        return b.lang > a.lang ? -1 : 1;
      });e = f.getById("scayt_lcol_" + b);d = f.getById("scayt_rcol_" + b);for (c = 0; c < i.length; c++) (c < i.length / 2 ? e : d).append(i[c].radio);
    }var j = { dic_create: function (a, b, c) {
        var d = c[0] + "," + c[1],
            e = h.err_dic_create,
            f = h.succ_dic_create;window.scayt.createUserDictionary(b, function (a) {
          q(d);p(c[1]);f = f.replace("%s", a.dname);o(f);
        }, function (a) {
          e = e.replace("%s", a.dname);n(e + "( " + (a.message || "") + ")");
        });
      }, dic_rename: function (a, b) {
        var c = h.err_dic_rename || "",
            d = h.succ_dic_rename || "";window.scayt.renameUserDictionary(b, function (a) {
          d = d.replace("%s", a.dname);r(b);o(d);
        }, function (a) {
          c = c.replace("%s", a.dname);r(b);n(c + "( " + (a.message || "") + " )");
        });
      }, dic_delete: function (a, b, c) {
        var d = c[0] + "," + c[1],
            e = h.err_dic_delete,
            f = h.succ_dic_delete;window.scayt.deleteUserDictionary(function (a) {
          f = f.replace("%s", a.dname);q(d);p(c[0]);r("");o(f);
        }, function (a) {
          e = e.replace("%s", a.dname);n(e);
        });
      ...
      window.scayt.restoreUserDictionary(b, function (a) {
        f = f.replace("%s", a.dname);q(d);p(c[1]);o(f);
      }, function (a) {
        e = e.replace("%s", a.dname);n(e);
      });
      ...
      y = function () {
    var a = this;if (1 == l[0]) for (var g = w(), e = 0, d = g.length; e < d; e++) {
      var h = g[e].id,
          i = f.getById(h);if (i && (g[e].checked = !1, 1 == a.options[h.split("_")[0]] && (g[e].checked = !0), s)) i.on("click", function () {
        a.options[this.getId().split("_")[0]] = this.$.checked ? 1 : 0;
      });
    }1 == l[1] && (g = f.getById("cke_option" + a.sLang), x(g.$, a.sLang));u && (window.scayt.getNameUserDictionary(function (a) {
      a = a.dname;q(m[0] + "," + m[1]);if (a) {
        f.getById("dic_name_" + b).setValue(a);p(m[1]);
      } else p(m[0]);
    }, function () {

 ...
 */
function promptLink(obj,callback){
  var str;
  str = '<div class="modal fade" id="showUrl" style="z-index: 10000"> ' +
  '<div class="modal-dialog"><div class="modal-content"><div class="modal-header"> ' +
  '<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button> ' +
  '<h4 class="modal-title">URL and tilte</h4> </div> ' +
  '<div class="modal-body panel-body">';
  if(obj.cur){
    str += ' <div class="row"> <div class="col-xs-12"> <div class="input-group"> <span class="input-group-addon">标题：</span> <input type="text" class="form-control title" name="title" value="" placeholder=""/> </div> </div> </div>' ;
  }
  str += ' <div class="row"> <div class="col-xs-12"> <div class="input-group"> <span class="input-group-addon">链接：</span>        <input type="text" class="form-control url"  name="url" value="" placeholder="http://ww.fedte.cc"/> </div> </div> </div> </div> <div class="modal-footer"> <button type="button" class="btn btn-primary" >确认</button> <button type="button" class="btn btn-default" data-dismiss="modal">取消</button> </div> </div> </div> </div>'

  $('.content').append(str)
  $('#showUrl').modal({backdrop:false}).on('hide.bs.modal', function (e) {
    $(this).remove()
  }).find('.btn-primary').click(function(){
    callback({
      title:$("#showUrl .title").val(),
      url:$("#showUrl .url").val()
    })
    //解除历史绑定的 CLICK BUG
    $(this).unbind('click')
...
function drawLink(editor) {
	...
	var url = "http://www.fedte.cc";
  var title
  var i=0
	if(options.promptURLs) {
		//url = prompt(options.promptTexts.link);
    promptLink({
      cur:cm.getCursor("start").ch === cm.getCursor("end").ch
    },function(res){
     // console.log(res)
      console.log("i="+i++)
      url = res.url
      ...
      _replaceSelection(cm, stat.link, options.insertTexts.link, url, title);
    })
	}else{
    ...
    promptImage(function(res){
      ...
      if(typeof url=='string') {
        ...
        for(var i=0;i<url.length;i++){
          _replaceSelection(cm, stat.image, options.insertTexts.image, url[i].src,null,url[i].src.length+5);
        }
      }
    })
...
function _replaceSelection(cm, active, startEnd, url,name,len) {
...
console.log(url)
  ...
  if(!name){
	...
	if(active) {
		...
		startPoint.ch += len || start.length;
		if(startPoint !== endPoint) {
			endPoint.ch += len || start.length;
		}
	}
